...
namespace Tests
{
    public class NonConvertibleTests : IClassFixture<NonConvertibleFixture>
	{
		private readonly NonConvertibleFixture fixture;

		public NonConvertibleTests(NonConvertibleFixture fixture)
		{
		...
		public void ExtensionsRetrieveRelatedNodes()
		{
			...
			Assert.Equal(new NonConvertible[] { root }, root.WithParent(x => x.Parent));
			Assert.Equal(new NonConvertible[] { grandchild, grandchild.Parent }, grandchild.WithParent(x => x.Parent));

			var rootWithChildren = new List<NonConvertible>() { root };
			...
			var descendants = new List<NonConvertible>() { root };
			...
			var ancestors = new List<NonConvertible>() { grandchild, firstChild, root };
